...
describe("date and billing helpers", () => {
	...
	it("returns a finite percentage for a zero-length billing period", () => {
		const result = getDaysBetweenDatesWithAutoRenewal({
	...
	it("rejects invalid cycle calculations", () => {
		...
		expect(() => getDaysBetweenDates("invalid", "2025-01-15")).toThrow(
			"参数无效",
		);
	});
...
describe("public note helpers", () => {
	...
	it("normalizes malformed public note fields without leaking runtime types", () => {
		expect(
			parsePublicNote(
				JSON.stringify({
					billingDataMod: ["unexpected"],
					planDataMod: {
						bandwidth: 1000,
						networkRoute: { route: "CN2" },
						extra: ["Backup"],
					},
				}),
			),
