...
const KNOWN_FONT_LOGOS = new Set([
	"almalinux",
	"alpine",
	"aosc",
	"apple",
...
export function GetFontLogoClass(platform: string): string {
	...
	if (KNOWN_FONT_LOGOS.has(normalized)) return normalized;
	if (normalized === "darwin") {
		return "apple";
	}
	if (["openwrt", "linux", "immortalwrt"].includes(normalized)) {
		return "tux";
	}
	...
	if (normalized.includes("opensuse")) {
		return "opensuse";
	}
	return "tux";
}
...
export function GetOsName(platform: string): string {
	const normalized = platform.toLowerCase();
	if (KNOWN_FONT_LOGOS.has(normalized)) {
